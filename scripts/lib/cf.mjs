...
export function getToken(mode = 'read') {
  if (mode === 'edit') {
    ...
    if (!t) throw new Error('CF_EDIT_TOKEN not set — break-glass edit token required. See docs/SAFETY.md')
    return t
  }
  const t = process.env.CF_READ_TOKEN || process.env.CF_API_TOKEN
  if (!t) throw new Error('CF_READ_TOKEN not set. Copy .env.example → .env and add a read-only token. See docs/TOKEN-SETUP.md')
  return t
}
