...
const ENV_FILE = DIRS.env

// ── salt ──────────────────────────────────────────────────────────────────
/** Load CF_ALIAS_SALT from the environment, or mint + persist one into the scoped env file. */
export function ensureSalt() {
  if (process.env.CF_ALIAS_SALT) return process.env.CF_ALIAS_SALT
  if (!existsSync(ENV_FILE)) {
    throw new Error(`No Cloudflare env found at ${ENV_FILE}. Run the setup command first.`)
  }
  ...
  log.warn(`CF_ALIAS_SALT was missing; generated one in ${ENV_FILE}. Back it up with secrets/alias-map.json.`)
  return salt
}
